...
func place_piece() -> void:
	...
	var possible_placements = get_possible_placements(current_player_tile)
	
	if possible_placements.is_empty():
		print("AI couldn't find a valid position to place the piece.")
		return
	
	var best_placement = choose_best_placement(possible_placements)
	
	tile_map.place_piece(best_placement.position, current_player_tile)
		...
		for x in range(board_rect.position.x, board_rect.end.x):
			...
			if tile_map.can_place_piece(position, current_player_tile):
				placements.append(
					{
						"position": position,
						"score": evaluate_placement(position, current_player_tile)
					}
				)
...
func evaluate_placement(position: Vector2i, current_player_tile: Vector2i) -> float:
	var score = 0.0
	
	for offset in tile_map.active_piece:
		var tile_position = position + offset
		if tile_map.get_cell_atlas_coords(tile_map.BOARD_LAYER, tile_position) == tile_map.EMPTY_TILE:
			score += 1.0
	
	var opponent_tile = tile_map.PLAYER_2_TILE if current_player_tile == tile_map.PLAYER_1_TILE else tile_map.PLAYER_1_TILE
	for offset in tile_map.active_piece:
		var tile_position = position + offset
		if tile_map.get_cell_atlas_coords(tile_map.BOARD_LAYER, tile_position) == opponent_tile:
			score += 2.0
